  ...
  return (
    <AuthGuard>
      ...
      <main className="mx-auto max-w-7xl px-6 pb-8">
        ...
        <section className="rounded-[2.5rem] border border-white/10 bg-[#0D1820]/90 p-12 backdrop-blur">
          <div className="mb-10">
            <p className="mb-3 text-xs uppercase tracking-[0.4em] text-[#7D837A]">
              Journey Archives
            </p>
            ...
            <p className="mt-3 max-w-3xl text-base text-[#A5ABA3]">
              Your latest personalizations presented so you can refine or book with ease whenever inspiration strikes.
            </p>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {recentTours.map((tour, index) => (
              <div
                key={index}
                className="group rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur transition hover:border-[#FFE5B4]/45 hover:bg-white/10"
              >
                <div className="mb-4">
                  <span className="inline-block rounded-full border border-[#FFE5B4]/30 bg-[#FFE5B4]/10 px-3 py-1 text-xs font-medium uppercase tracking-wider text-[#FFE5B4]">
                    {tour.status}
                  </span>
                </div>
                <h3 className="mb-4 text-lg font-semibold text-white group-hover:text-[#FFE5B4]">
                  {tour.name}
                </h3>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between text-[#D0D7D8]">
                    <span className="text-[#7D837A]">Dates</span>
                    <span>{tour.dates}</span>
                  </div>
                  <div className="flex justify-between text-[#D0D7D8]">
                    <span className="text-[#7D837A]">Travelers</span>
                    <span>{tour.travelers}</span>
                  </div>
                  <div className="flex justify-between text-[#D0D7D8]">
                    <span className="text-[#7D837A]">Budget</span>
                    <span className="font-semibold text-[#FFE5B4]">{tour.budget}</span>
                  </div>
                </div>
                <button className="mt-6 w-full rounded-xl border border-white/20 bg-white/5 py-2.5 text-sm font-medium text-white transition hover:border-[#FFE5B4]/50 hover:bg-white/10">
                  {tour.status === "Completed" ? "Continue Booking" : tour.status === "Saved" ? "Review Itinerary" : "Share Itinerary"}
                </button>
              </div>
            ))}
          </div>
