...
export default function FoodSearch() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(error.message || 'Không thể tìm kiếm nhà hàng. Thử lại sau.', 'Lỗi tìm kiếm')
    } finally {
      setIsLoading(false)
    }
  }
          ...
          <div className="mt-8">
            ...
            <div className="space-y-4">
              {results.map((place) => {
                ...
                return (
                  <div key={place.id} className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-green-50 overflow-hidden">
                    {/* Card Header - Clickable */}
                    <div
                      onClick={() => togglePlace(place.id)}
                      className="cursor-pointer p-5 transition-all hover:bg-emerald-100/50"
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-[#A1A1AA]">avg. meal</p>
                        </div>
                      </div>
