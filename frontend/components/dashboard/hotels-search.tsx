...
export default function HotelsSearch() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const filtered = data.hotel.filter(
          ...
          <div className="mt-8">
            ...
            <div className="space-y-4">
              {results.map((place) => {
                ...
                return (
                  <div key={place.id} className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-gray-50 overflow-hidden">
                    {/* Card Header - Clickable */}
                    <div
                      onClick={() => togglePlace(place.id)}
                      className="cursor-pointer p-5 transition-all hover:bg-slate-100/50"
                    >
